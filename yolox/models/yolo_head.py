...
class YOLOXHead(nn.Module):
    def __init__(
        ...
        self.decode_in_inference = False  # for deploy, set to True
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        if self.training:
            ...
            if self.decode_in_inference:
                ...
                return torch.cat(yolo_outputs, dim=2)
            else:
                self.hw = [x.shape[-2:] for x in outputs]
                outputs = torch.cat(
                    [x.flatten(start_dim=2) for x in outputs], dim=2
    ...
    def decode_output_mobile_platform(self, output, ny, nx, stride, dtype, save_file):
        ...
        grid = torch.stack((xv, yv), 2).view((1, ny, nx, 2)).float()
        save_file.write(str(ny) + '\t' + str(nx) + '\t' +  str(stride) + '\n')
        #xy, wh, obj, cls = torch.split(output, [2, 2, 1, 1], 4) 
        #xy, wh, obj, cls = torch.split(output, [2, 2, 1, self.num_classes], 4) 
        #xy, wh, obj, cls = torch.split(output, [2, 2, 1, self.num_classes], 3) 
        #xy = (xy + grid) * stride
        #wh = torch.exp(wh) * stride
        #x = torch.cat([xy, wh, obj, cls], 4)
        #x = torch.cat([xy, wh, obj, cls], 3)
        #x = x.view(1, 1, ny*nx, 5 + self.num_classes)
        x = output.view(1, 1, ny*nx, 5 + self.num_classes)
        return x
    ...
    def get_losses(
        ...
        #loss_obj = (
        #    self.focal_loss_discrite(obj_preds.sigmoid().view(-1, 1), obj_targets)
        #).sum() / num_fg
        loss_cls = (
