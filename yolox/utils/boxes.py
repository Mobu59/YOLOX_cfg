...
from .nms_utils import set_cpu_nms
...
def postprocess(prediction, num_classes, conf_thre=0.7, nms_thre=0.45, class_agnostic=False):
    ...
    for i, image_pred in enumerate(prediction):
        ...
        conf_mask = (image_pred[:, 4].squeeze() >= conf_thre).squeeze()
        ...
        if class_agnostic:
            ...
            #nms_out_index = set_cpu_nms(detections, nms_thre)
        else:
...
def bboxes_iou_batch(bboxes_a, bboxes_b, xyxy=True):
    ...
    if xyxy:
        ...
        area_a = box_a_w * box_a_h
        area_a = torch.reshape(area_a, (N, A, 1))
        #area_a = torch.tile(area_a, (1, 1, B))
        area_a = area_a.repeat((1, 1, B))

        box_b_w = box_b[:, :, 2] - box_b[:, :, 0]
        box_b_h = box_b[:, :, 3] - box_b[:, :, 1]
        area_b = box_b_w * box_b_h
        area_b = torch.reshape(area_b, (N, 1, B))
        #area_b = torch.tile(area_b, (1, A, 1))
        area_b = area_b.repeat((1, A, 1))
