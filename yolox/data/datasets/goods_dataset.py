...
from .goods_classes import GOODS_CLASSES
...
class AnnotationTransform(object):
    ...
    def __init__(self, class_to_ind=None, keep_difficult=True):
        self.class_to_ind = class_to_ind or dict(
            ...
            zip(GOODS_CLASSES, range(len(GOODS_CLASSES)))
        )
...
class GoodsDataset(Dataset):
    ...
    def load_resized_img(self, index):
        ...
        resized_img = np.zeros((416, 416, 3), dtype = np.uint8)
    ...
    def _write_voc_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(GOODS_CLASSES):
            cls_ind = cls_ind
    ...
    def _do_python_eval(self, output_dir="output", iou=0.5):
        ...
        if output_dir is not None and not os.path.isdir(output_dir):
            os.mkdir(output_dir)
        for i, cls in enumerate(GOODS_CLASSES):
